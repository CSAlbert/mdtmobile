...
public class CIEDRankingDao {
    ...
    public String getLogonToken() throws SapBoException {
        ...
        try {
            ...
            throw new SapBoException("在CIEDRankingDao中调用getLogonToken方法出错。出错信息：url:" + boUrl +
                    "/biprws/logon/long", e);
        } catch (IOException e) {
            throw new SapBoException("在CIEDRankingDao中调用getLogonToken方法出错。用户信息：" + userString + "; url:" + boUrl +
                    "/biprws/logon/long", e);
        } finally {
            try {
                //释放资源
    ...
    public JSONObject getDocumentIdAndDescription(String logonToken, String cuid) throws SapBoException, ParameterException {
        ...
        // 检查参数合法性
        if (logonToken == null || logonToken.length() == 0|| cuid == null || cuid.length() == 0){
