  ...
  var jw = jw || {
    ...
    config: function( params ){
      ...
      if(/iPhone|iPad|iPod/i.test(userAgent)){
        ...
        // Android 显示初始化，通知Native
        var WVJBIframe = document.createElement('iframe');
        WVJBIframe.style.display = 'none';
        WVJBIframe.src = 'jw://resource/nativejssdk';
        document.documentElement.appendChild(WVJBIframe);
        setTimeout(function() { document.documentElement.removeChild(WVJBIframe) }, 0)
    ...
    listen:function(bridge){
      ...
      bridge.registerHandler("onVoiceRecordEnd", function(data, responseCallback) {
        if( typeof (window.onJWVoiceRecordEnd) == 'function' ) window.onJWVoiceRecordEnd(data);
      });
      bridge.registerHandler("onSearchBeacons", function(data, responseCallback) {
        if( typeof (window.onSearchBeacons) == 'function' ) window.onSearchBeacons(data);
      });
      bridge.registerHandler("onImageSelected", function(data, responseCallback) {
        if( typeof (window.onImageSelected) == 'function' ) window.onImageSelected(data);
      });
      bridge.registerHandler("onBase64ImageSelected", function(data, responseCallback) {
        if( typeof (window.onBase64ImageSelected) == 'function' ) window.onBase64ImageSelected(data);
      });
    },
    ...
    showTabs: function(origindata){
      ...
      if(typeof origindata['focusidx']!='undefined')
        data['focusidx'] = origindata['focusidx'];
      JWBridge('showTabs', data, origindata );
    },
    chooseTab: function( origindata ){
      JWBridge('chooseTab', origindata, origindata );
    },
    ...
    ocr:function(data,origindata){
      JWBridge('ocr',data,origindata);
    },
    startRecord:function(origindata){
      JWBridge('startRecord',{},origindata);
    },
    stopRecord:function(origindata){
      JWBridge('stopRecord',{},origindata);
    },
    chooseUsers:function(data,origindata){
      JWBridge('chooseUsers',data,origindata);
    },
    objSelector:function(data,origindata){
      JWBridge('objSelector',data,origindata);
    },
    getDeviceID:function(origindata){
      JWBridge('getDeviceID',{},origindata);
    },
    getDevice:function(origindata){
      JWBridge('getDevice',{},origindata);
    },
    shareObj:function(data,origindata){
      JWBridge('shareObj',data,origindata)
    },
    panoramic:function(data,origindata){
      JWBridge('panoramic',data,origindata)
    },
    startSearchBeacons:function(data,origindata){
      JWBridge('startSearchBeacons',data,origindata);
    },
    stopSearchBeacons:function(origindata){
      JWBridge('stopSearchBeacons',{},origindata);
    },
    onSearchBeacons:function(origindata){
      JWBridge('onSearchBeacons',{},origindata);
    },
    shareMessage:function(data,origindata){
      JWBridge('shareMessage',data,origindata);
    },
    startShareLocation:function(data,origindata){
      JWBridge('startShareLocation',data,origindata);
    },
    stopShareLocation:function(origindata){
      JWBridge('stopShareLocation',{},origindata);
    },
    getObjMembers:function(data,origindata){
      JWBridge('getObjMembers',data,origindata);
    },
    chooseBase64Image:function(data,origindata){
      JWBridge('chooseBase64Image',data,origindata);
    },
    getFullInfo:function(origindata){
      JWBridge('getFullInfo',null,origindata);
    },
    shareURL:function(data,origindata){
      JWBridge('shareURL',data,origindata);
    },
    openBrowser:function(data,origindata){
      JWBridge('openBrowser',data,origindata);
    },
    aPay:function(data,origindata){
      JWBridge('aPay',data,origindata)
    },
    getDepts:function(origindata){
      JWBridge('getDepts',null,origindata)
    },
  };
  return jw;
}));
